        ...
        for(name in source){
			if (source.hasOwnProperty(name)) {
	            ...
	            if(!(name in target) || (target[name] !== s && (!(name in empty) || empty[name] !== s))){
					//naive implementation of deep copy - recurse if not number, string, boolean, array, or function
					var t = typeof(s);
					if (t != 'string' && t != 'number' && t != 'boolean' && t != 'function' && !he.isArray(s)) {
						target[name] = mixin(target[name]||{},s);
					}
					else {
						target[name] = s;
					}
